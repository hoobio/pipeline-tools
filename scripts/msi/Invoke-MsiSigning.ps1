...
<#
.SYNOPSIS
Imports a PFX signing certificate and signs every MSI under a directory with signtool.
...
Two-step signing flow used in CI:
  ...
  2. Locate the newest x64 `signtool.exe` shipped with the Windows SDK, then sign every
     `.msi` under the supplied directory by certificate thumbprint.
...
Mirrors `scripts/msix/Invoke-MsixSigning.ps1` exactly except for the file extension.
signtool itself supports MSI without any additional flags (the `/fd` digest applies
the same way).
...
Directory containing one or more .msi files. Subdirectories are searched.
#>
[CmdletBinding()]
param(
...
$packages = Get-ChildItem -Path $PackagesPath -Recurse -Filter *.msi -ErrorAction SilentlyContinue
if (-not $packages) {
    throw "No .msi files found under '$PackagesPath'"
}
